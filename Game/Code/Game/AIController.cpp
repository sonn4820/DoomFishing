...
ActorUID AIController::FindClosestEnemy()
{
	...
	if (closestActors.IsValid())
	{
		...
		else
		{
			...
			float angleBetween = GetAngleDegreesBetweenVectors3D(m_controllerActorUID->GetActorForward(), (closestActors->m_position - m_controllerActorUID->m_position).GetNormalized());
